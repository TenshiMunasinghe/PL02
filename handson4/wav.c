...
bool note_write(song s, int frequency, int duration, int velocity)
{
    ...
    n->duration = duration;
    n->velocity = velocity; // Store velocity with the note
...
{
    return note_write(s, 0, duration, 100); // Default velocity for rests
}
...
bool song_close(song s)
{
    ...
    for (size_t i = 0; i < s->size; i++)
    {
        ...
        // Scale volume based on velocity (1-127 to 0.0-1.0)
        double volume_scale = (double)n->velocity / 127.0;

        for (; current_sample != note_end; phase += phase_step)
        {
            *current_sample++ = round(VOLUME * volume_scale * sin(phase));
        }
