...
{
	printf("%d:%d:%d", time.day, time.hours, time.minutes);
}
...
int isTimeValid(Time time)
{
	...
	// ...
	if (time.hours < 0 || time.hours > HOURS_IN_DAY || time.minutes < 0 || time.minutes > MINUTES_IN_HOUR)
	{
		return 0;
	}
	return 1;
