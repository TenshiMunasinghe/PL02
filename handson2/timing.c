...
void printTime(Time time) {
  printf("%02d:%02d:%02d", time.day, time.hours, time.minutes);
}
...
void printTimeSpan(TimeSpan time) {
  ...
  duration.minutes = time.minutes % MINUTES_IN_HOUR;
...
int isTimeValid(Time time) {
  ...
  // ...
  if (time.hours < 0 || time.hours > HOURS_IN_DAY || time.minutes < 0 ||
